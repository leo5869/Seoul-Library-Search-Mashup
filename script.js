...
const apiKey = "677369696673616d3839556d7a6351";
...
function addBound(radius){
	...
	// 지도에 원 표시 
	bound.setMap(map);
	daum.maps.event.addListener(map, 'dragstart', function() { // 지도가 이동될 때도 원이 다시 그려짐
		bound.setMap(null);
	});
}
...
function updateLibrary(libraries) {//16번 호출
 ...
 function addMarker(imageSrc, imageSize, imageOption, latitude, longitude, name, address,tel, closeday) {
	...
	daum.maps.event.addListener(map, 'dragstart', function() {
		...
		var content = "<div class='bg-primary text-white' style='width:100%; height:100%; padding:5px; font-size:1.2rem;'>"+
...
function computeDistance  (startCoords,destCoords){ // 거리계산
